...
from deepctr_torch.inputs import SparseFeat, DenseFeat, get_feature_names, embedding_lookup
...
class MOE_DIN(MyBaseModel):
    def __init__(self,
                 linear_feature_columns, dnn_feature_columns, history_feature_list, use_fm=False, use_nfm=False,
        ...
        super(MOE_DIN, self).__init__([], dnn_feature_columns, l2_reg_linear=l2_reg_linear,
        ...
        if self.pretrained_user:
            ...
            # self.embedding_dict['userid'].weight.data.copy_(torch.from_numpy(pretrained_user_emb_weight[0]))
            # self.embedding_dict['userid'].weight.requires_grad = True
        ...
        if self.pretrained_author:
            ...
            self.embedding_dict['hist_authorid'].weight.data.copy_(torch.from_numpy(pretrained_author_emb_weight[0]))
            self.embedding_dict['hist_authorid'].weight.requires_grad = True
        ...
        if self.pretrained_feed:
            ...
            self.embedding_dict['hist_feedid_pre1'].weight.data.copy_(torch.from_numpy(pretrained_feed_emb_weight[0]))
            self.embedding_dict['hist_feedid_pre1'].weight.requires_grad = True

            self.embedding_dict['hist_feedid_pre2'].weight.data.copy_(torch.from_numpy(pretrained_feed_emb_weight[1]))
            self.embedding_dict['hist_feedid_pre2'].weight.requires_grad = True
        ...
        ############DIN Part
        hist_act_pre = 'hist_'
        # din_feature_names = [n for n in history_feature_list]
        # din_feature_names = [hist_act_pre+n for n in history_feature_list]
        # din_feature_columns = [col for col in dnn_feature_columns if col.name in din_feature_names]
        din_feature_columns = self.varlen_sparse_feature_columns
        self.din = my_DIN(din_feature_columns, history_feature_list, self.embedding_dict, self.feature_index, hist_act_pre)
        dnn_input_dim += self.compute_input_dim(din_feature_columns)
        ############DIN Part
        ...
        for i in range(self.num_tasks):
            if (i==5):
                ...
                dnn_linear.append(nn.Linear(dnn_hidden_units[i][-1]*3, 1, bias=False))
            else:
    ...
    def forward(self, X):
        ...
        hist_embed, query_emb_list = self.din(X)
        dnn_input_emb_list = dnn_input_emb_list+query_emb_list
        dnn_input_emb_list.append(hist_embed)
        ...
        if self.pretrained_author:
            for emb in self.author_emb:
                pretrained_emb.append(emb(X[:,self.feature_index['hist_authorid'][0]].long()))
        if self.pretrained_feed:
            for emb in self.feed_emb:
                # pretrained_emb.append(emb(X[:,self.feature_index['feedid'][0]].long()))
                pretrained_emb.append(emb(X[:,self.feature_index['hist_feedid'][0]].long()))
        ...
        dnn_input = combined_dnn_input(dnn_input_emb_list, dense_value_list)
        ...
        # 相关性较强的几个任务, dnn最后一层的输出可以和相关任务输入合并输入最后一层 !!!!!!!!!!!!
        # INDEX :[read_comment, like, click_avatar, forward, favorite, comment, follow]
        # read_comment & comment / comment = 0.5585950692333671 =>  read_comment可以作为comment的输入
